...
\begin{theorem}
    ...
    \begin{proof}
        ...
        We need to prove the convergence of $x_n$ and $u_n$. {Because $U$ is compact and \textit{convex} then $L^{\infty}([t,t_1];U)$ is 
...
We could consider more general $f(\cdot,\cdot,\cdot)$ and still get existence, indeed as proven in Cannarsa\cite[Cannarsa] the key conditions on $f$ is for the

\[f(t,x,U)=\left\{f(t,x,v):\, v\in U\right\}\]

to be convex. Indeed, the Following holds true.

\begin{theorem}
    Let $U$ compact and $f$ Lipschitz with respect to the state variable and such that $f(t,x,U)$ is convex for all $(t,x)$, then there exists an optimal control for all $(t,x)$.
\end{theorem}