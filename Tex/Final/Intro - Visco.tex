...
therefore, if we maximize $J$ for each values of $\tau$ we get $v=0,2,-2$ respectively.  In order to assign each $v$ a domain we observe that if $\abs{x}<t$ then neither $v=2$ nor $v=-2$ are enough for $x(\cdot)$ to optimally exit from $[-1,1]$ before $s=1$, and thus it is cost minimizing not to move ($v=0$), while if $x\geq t$ or $x\leq -t$ it is worth and we respectively get $v=2$ and $v=-2$. Thus,
...
calculus of variations problems. It will show us one of the origins of the viscosity 
...
real line, thus a weaker notion of solution is needed. One approach is to use Hopf-Lax solution concept. We are not 
interested in it, instead we search for solution from similar problems which we already know. Let us perturbate the system as

\begin{equation}\label{4-1-semilinear}
    \begin{cases}
...
so that the fully non-linear system in \ref{4-1-fullynonlinear} becomes a semilinear one, which has a smooth solution, e.g. Evans\cite{evans10} illustrates the Galerkin approximation method. Since the system \eqref{4-1-fullynonlinear} is some sort of limit version of \eqref{4-1-semilinear} we are tempted to take the limit of its solutions as $\epsilon\to 0$ and see if, provided convergence, we get the solution.  
Thus, we take $\epsilon\to0$. We expect the solution $u^{\epsilon}$ to lose the bounds on the derivatives, as they strongly depend on the 
...
Let us take $v\in C^{\infty}(\R^n\times(0,+\infty))$ and suppose that $u-v$ has a strict local maximum at $(x_0,t_0)$ and $u(x_0,t_0) = v(x_0,t_0)$, then
...
Because $(u^{\epsilon}-v)$ has a local maximum at $(x_{\epsilon_j},t_{\epsilon_j})$ then
...
\begin{definition}
    A viscosity solution of \ref{4-1-fullynonlinear} is a function $u$ bounded and uniformly continuous on $\R^n\times[0,T]$ for all $T>0$ such that $\forall v\in C^{\infty}(\R^n\times(0,+\infty))$
...
\section{Abstract dynamic programming}
...
equation. Let $\Sigma$ be a closed subset of a Banach space and $\mathcal{C}$ a collection of functions on $\Sigma$, closed under addition, that is
...
Moreover, they also imply the semigroup property, provided that $\mathcal{T}_{rt}:\mathcal{C}\rightarrow\mathcal{C}$. If the latter holds true,
...
will mimic the dynamic programming principle. Indeed, let us consider the classical optimal control problem defined on a bounded set 
$O\subset\R^n$, which we choose to be $\Sigma=\overline{O}$ and $\mathcal{C}=\mathcal{M}(\Sigma)$, the collection of measurable functions bounded by below. Then, as in chapter 1, we aim at minimizing a functional, 
...
Under the usual assumption on the running and terminal costs as well as on the control space $U$, we note that the value function defined in \ref{4-1-valuefunctreform} 
...
It is clear by its definition that the dynamic programming principle is
...
We now derive the abstract dynamic programming equation. The same procedure as in chapter 1 yields
...
to weaken this notion of solution and get the viscosity solution of the dynamic programming equation.

We point out that in deterministic optimal control, the infinitesimal generator has the form
...
for every $\overline{x}\in\arg \max \{(\phi-\psi)(x)\,|\,x\in\Sigma\}\cap\Sigma'$ with $\phi(\overline{x})=\psi(\overline{x})$. If $W$ is a classical solution of \ref{4-1-newprogreq}, then for a $w\in\mathcal{D}$ and $\left(\overline{t},\overline{x}\right) \in \arg \max_{(t,x)\in Q}\left\{(W-w)(t,x)\right\}$ 
and $W\left(\overline{t},\overline{x}\right)=w\left(\overline{t},\overline{x}\right)$ we have
...
\begin{proposition}
    ...
    \begin{proof}
        If $W$ is a viscosity solution, since it is also a test function then \ref{4-1-defsubsolution} and \ref{4-1-defsupersolution} hold for every point $(t,x)\in Q$, 
        which implies that
...
We have talked about \textit{viscosity solutions} for $\mathcal{G}_t$ that are infinitesimal generators of semigroups. The notion of viscosity solution was introduced 
...
establishing as well the relevant links to what we have already proven. 
...
A handy way of verifying both the definitions is the following.
...
\begin{proof}
    ...
    \begin{align*}
        \frac{1}{h}\left[(\mathcal{T}_{tt+h}w(t+h,\cdot))(x)-w(t,x)\right] & \leq \frac{1}{h}\int_t^{t+h}L(s,x(s),u(s))\,ds \\
        & \quad + \frac{1}{h}\left[w(t+h,x(t+h))-w(t,x)\right] \\
    ...
    \begin{equation}\label{miao}
        \begin{aligned}
            n\int_t^{t+1/n} L(s,x(s),u^n(s))\,ds + n\int_t^{t+1/n} f(t,x,u^n(s))\,ds \cdot D_x w(t,x) \\
            \leq \inf_{v\in U}\left\{f(t,x,v)\cdot p + L(t,x,v)\right\},
        \end{aligned}
    \end{equation}  
    ...
    % \[FL(t,x)\left\{(f(t,x,v),L(t,x,v))\,|\, v\in U\right\}\]
...
\begin{enumerate}\label{4-2-Hassumptions}
...
Under these hypotheses we can state the following theorem.
...
\begin{proof}
    ...
    \begin{enumerate}[label=\arabic*')]
        ...
        By the same reasoning we define the test function
...
We now investigate continuity properties of the value function. In particular, we consider two different sets of assumptions regarding continuity: Lipschitz continuity and Uniform continuity. Let us consider $f,L,\psi$ as usual, recalling that there exists $K_{\rho}>0$ such that 
...
\begin{theorem}\label{4-3-Theorem:1cont}
    ...
    \begin{proof}
        ...
        $V$ is uniformly continuous. Standard arguments on solutions of ODE that we have already used many times (integral form of the solution, bound on the flow and Gronwall) yield 
        ...
        \begin{align*}
            \abs{J(t,x,u) - J(t,y,u)} & \leq \int_t^{t_1} \abs{L(s,x(s),u(s)) - L(s,y(s), u(s))}\,ds \\
            & \quad + \abs{\psi(x(t_1)) - \psi(y(t_1))} \\
            & \leq \int_t^{t_1} m_L(\Lambda(s))\,ds + m_{\psi}(\Lambda(t_1)) \\
            & \quad + (t_1-t_0)m_L(\abs{x-y}e^{K(t_1-t_0)}) + m_{\psi}(\abs{x-y}e^{K(t_1-t_0)}),
        \end{align*}
...
It is now clear that value functions may easily fail to be even differentiable, and thus the necessity of a weaker notion of solution. We have introduced 
the concept of viscosity solution, which turns out to be a key concept to extend Pontryagin's principle in non-differentiable settings. 
Recall that Pontryagin states the relation between a so-called adjoint variable $P$ and the state variable via a system of differential equations. Since the Hamiltonian may fail 
to be differentiable, we need to carefully extend the possible values of the adjoint variable for those instances. Let us 
...
\begin{example}
    ...
    Let us compute $D^+V(t,x)$ and $D^-V(t,x)$. If $(t,x)$ are not of the form $t=\abs{x}$ the value function is differentiable and the superdifferential and subdifferential coincide with the derivative. Let $a\in[0,1]$ let us compute $D^+V(a,a)$, we compute all the gradients the function can take, the superdifferential will be their convex. When \( x > 0 \) at \( t = x \), \( V(t, x) = 1 - t \). The partial derivatives when approaching from different sides are:
    \[
    \frac{\partial V}{\partial t} = -1 \quad \text{(approaching from the \( t < |x| \) side)}.
    \]
    \[
    \frac{\partial V}{\partial x} = -1 \quad \text{(approaching from the \( t > |x| \) side)}.
    \]
    
    When \( x < 0 \), at \( t = -x \), \( V(t, x) = 1 - t \). The partial derivatives when approaching from different sides are:
    \[
    \frac{\partial V}{\partial t} = -1 \quad \text{(approaching from the \( t < |x| \) side)}.
    \]
    \[
    ...
    The superdifferential is given by the convex combination of these gradients:
    \[
    D^+ V(t, x) = \left\{ \lambda(-1, 0) + (1 - \lambda)\left(0, -\frac{x}{|x|}\right) : \lambda \in [0, 1] \right\}.
    \]
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \frac{\partial}{\partial x_i}J(s,x^{\ast}(s);u^{\ast}) & = \sum_{j=1}^n\Bigg\{\int_s^{t_1} L_{x_j}(r,x^{\ast}(r),u^{\ast}(r))\frac{\partial}{\partial x_j}x_i(r,x^{\ast}(r))\,dr \\
            & \quad + \psi_{x_j}(x^{\ast}(t_1))\frac{\partial}{\partial x_i}x_j(t_1,x^{\ast}(t_1))\Bigg\},
        \end{align*}
        ...
        \begin{align*}
            ...
            & \quad + \sum_{j=1}^n\Bigg\{\int_r^{t_1}\frac{\partial}{\partial x_j}L(s,x^{\ast}(s),u^{\ast}(s))z_j(s;r,x^{\ast}(r))\,ds \\
            & \quad + \frac{\partial}{\partial x_j}\psi(t_1,x^{\ast}(t_1))z_j(t_1;r,x^{\ast}(r))\Bigg\},
        \end{align*}
        ...
        \begin{align*}
            -f(r,x^{\ast}(r),u^{\ast}(r))\cdot p(r) & = z(r)\cdot p(r)\\
            & = z(t_1)\cdot p(t_1) - \int_r^{t_1} \frac{d}{ds}[z(s)\cdot p(s)]\,ds \\
            & = \sum_{j=1}^n\Bigg\{\frac{\partial}{\partial x_j}\psi(t_1,x^{\ast}(t_1))z_j(t_1) \\
            & \quad + \int_r^{t_1} \frac{\partial}{\partial x_j}L(s,x^{\ast}(s),u^{\ast}(s))z_j(s)\,ds\Bigg\},
        \end{align*}
