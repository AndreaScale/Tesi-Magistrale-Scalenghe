...
\begin{lemma}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            If $s>r$ then $x(\cdot)$ and $x_{\epsilon}(\cdot)$ solve the same ODE with initial condition at $s$ given by\footnote{via first order approximation.}

            \[x_{\epsilon}(s) = x(s) + \epsilon (f(x(s),a) - f(x(s),u(s))) + o(\epsilon),\]

            thus, it can be shown\footnote{Evans} that this necessarily implies

            \[x_{\epsilon}(s) = x(s) + \epsilon y(s) + o(\epsilon).\]
        \end{itemize}
    \end{proof}
\end{lemma}
...
the problem has no running cost, thus we can apply the theorem and, noticing that 
