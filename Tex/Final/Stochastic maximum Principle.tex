...
We denote by $m$ the Lebesgue measure on $[t,t_1]$. We first need some preliminary results.
...
\begin{theorem}\label{2-3-stochmaxprinc}
    ...
    \begin{proof}
        ...
        \begin{equation}\label{Ito to V BSDE}
            \begin{aligned}
                V(s,x(s)) & = \int_t^{t_1} \partial_sV(r,x(r)) + D_xV(r,x(r))f(r,x(r),u(r)) \\
                & \quad + \frac{1}{2}tr\left[\sigma\sigma'(r,x(r),u(r))D_x^2V(r,x(r))\right] \,dr \\
                & \quad + \int_t^{t_1} D_xV(r,x(r))\sigma(r,x(r),u(r))\,dw_r + V(t,x) \\
                & = \int_t^{t_1} - f(r,x(r),u(r))\,dr + \int_t^{t_1}\alpha_r\,dw_r + V(t,x),
            \end{aligned}
        \end{equation}

        then taking the expectation and identifying the terms we get
